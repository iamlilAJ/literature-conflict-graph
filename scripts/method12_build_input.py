"""Method 12 stage 1 — build judge input for ALL 300 hyps in 540p-thaw1.

Same TF-IDF Atlas-quote retrieval pipeline as Method 3, applied to the
full production run. Output is identical-shape JSONL that run_method3_judge.py
consumes unchanged.
"""
...
def main():
    ...
    hyps = [json.loads(l) for l in open(RUN / "hypotheses_scored.jsonl") if l.strip()]
    print(f"hyps: {len(hyps)} | Atlas quotes corpus: {len(quotes)}",
          file=sys.stderr)

    out = []
    for h in hyps:
        v = tfidf(toks(hyp_text(h)), idf)
        sims = sorted(((cos(v, qv), i) for i, qv in enumerate(qvecs)),
                      reverse=True)
        ...
        out.append({
            ...
            "top_k_atlas": top,
            "utility": h.get("utility"),  # carry the existing scorer for correlation
            "anomaly_type": h.get("anomaly_type"),  # may be None
        })

    op = _REPO / "artifacts/atlas_test/method12_judge_input.jsonl"
