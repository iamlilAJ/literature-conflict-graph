...
CREATOR_SYSTEM_PROMPT = (
    ...
    "- GROUNDING (hard): NO FABRICATED NUMBERS. Cite a benchmark number ONLY when it "
    "appears verbatim in a cited claim. If the cited claims are qualitative, state the "
    "prediction as a measurable TARGET against a NAMED baseline (e.g. 'higher EM than "
    "the RAG baseline in claim cX') — NOT an invented percentage. NEVER attach a "
    "made-up number to a past method/baseline: a fabricated '52% cost reduction' or "
    "'0.420 EM' not present in the claims is a hard failure. Predictions about your "
    "NEW method are targets, not established facts.\n\n"
    "SHAPE DIVERSITY (critical): when you propose more than one method, make them "
    "STRUCTURALLY DIFFERENT research moves — do NOT make every one 'add an adaptive / "
    "gating / fusion component'. Span, where the cluster supports it:\n"
    "  (a) interior-optimum: a design with a knob that has a sweet spot — name the "
    "knob, the predicted optimum region, and why MORE of it eventually HURTS;\n"
    "  (b) mechanism-differential: name the causal mechanism and predict BOTH a regime "
    "where it works AND a named regime where it FAILS;\n"
    "  (c) scaling/transfer: how the benefit scales with a measurable property, or what "
    "happens porting it to a NAMED new task/dataset.\n\n"
...
SYSTEM_PROMPT_COARSE = (
    ...
    "- cite a number ONLY if it is present in the evidence; never invent a baseline "
    "result — use a named-baseline target instead\n"
    "- prefer a structural move with an interior optimum (a knob with a sweet spot), a "
    "differential (works-here / fails-there) prediction, or a scaling/transfer law — "
    "NOT merely 'combine A and B'\n\n"
...
SYSTEM_PROMPT_SYNTHESIZE = (
    ...
    "concrete minimal_test); carry over a benchmark number from FINE ONLY if FINE took "
    "it from a real cited claim — never introduce a new fabricated number\n"
    "- inherits ambition from COARSE (cross-community framing, unifying mechanism)\n"
    "- keeps FINE's structural shape (interior-optimum / works-here-fails-there / "
    "scaling-transfer) rather than flattening to a generic 'combine A and B' framing\n"
