...
SYSTEM_PROMPT = (
    ...
    "- method: string or null. MUST be the name of an ML method/technique/algorithm.\n"
    "  Positive examples: 'RAG', 'Chain-of-Thought', 'DPR', 'Self-Refine', "
    "'Tree-of-Thought', 'DPO', 'PPO', 'Toolformer'.\n"
    "  Set to null (do NOT put in `method`) if the string is any of:\n"
    "    * a MODEL name — these go in `model` (e.g. 'GPT-4', 'Llama-70B', "
    "'Qwen2.5-VL', 'o1-preview', 'Claude Sonnet 4', 'Gemini-1.5-Pro')\n"
    "    * a METRIC — these go in `metric_raw` / `metric` "
    "(e.g. 'Top-1 accuracy', 'pass@k', 'F1', 'BLEU', 'macro-average accuracy')\n"
    "    * a DESCRIPTIVE PHRASE that isn't a named method "
    "(e.g. 'comprehensive evaluation', 'LLM-based planner', 'multi-agent approach', "
    "'fine-tuning' alone without a named protocol, "
    "'Wikipedia search and question answering tools')\n"
    "    * EXTRACTION NOISE (e.g. 'other models', 'their intricate workflows')\n"
    "  When the paper introduces a NEW named method, copy the name verbatim "
    "(e.g. 'SpatialRGPT', 'KD-Encoder', 'Citation-Enhanced Generation (CEG)').\n"
    "  When the paper applies a known method to a new task, keep just the "
    "method name ('RAG' not 'RAG agent for medical QA').\n"
    ...
    "If `method` is null, set `canonical_method` to 'other'.\n"
    "- model: string or null. MUST be a model/system identifier (e.g., 'GPT-4', "
    "'Llama-70B', 'Qwen2.5-VL', 'Claude Sonnet 4'). Comma-list multiple if needed.\n"
