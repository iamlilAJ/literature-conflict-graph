...
from .models import Anomaly, Claim, Hypothesis, Paper, ScoreBreakdown
...
def _render_hypothesis(h: Hypothesis, scores: dict[str, ScoreBreakdown]) -> list[str]:
    ...
    if s is not None:
        ...
        lines.append("")
        lines.append("| explain | grounding | testability | novelty | discrim | cost | utility |")
        lines.append("|---|---|---|---|---|---|---|")
        lines.append(
            f"| {s.explain:.2f} | {s.grounding:.2f} | {s.testability:.2f} | "
            f"{s.novelty:.2f} | {s.discriminability:.2f} | {s.cost:.2f} | {s.utility:.2f} |"
        )
        lines.append("")
    return lines
