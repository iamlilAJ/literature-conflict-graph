...
def _novelty_score_cached(h_tokens: set[str], claim_tokens: list[set[str]]) -> float:
    if not claim_tokens:
        return 1.0
    max_sim = 0.0
    for tokens in claim_tokens:
        sim = jaccard(h_tokens, tokens)
        if sim > max_sim:
            max_sim = sim
    return 1.0 - max_sim


def _discriminability_score_cached(
    h: Hypothesis,
    peers: list[Hypothesis],
    hypothesis_tokens: dict[str, set[str]],
) -> float:
    others = [p for p in peers if p.hypothesis_id != h.hypothesis_id]
    if not others:
        return 1.0
    h_tokens = hypothesis_tokens[h.hypothesis_id]
    sims = [jaccard(h_tokens, hypothesis_tokens[o.hypothesis_id]) for o in others]
    mean_sim = sum(sims) / len(sims)
    return 1.0 - mean_sim
...
def score_all(
    ...
    claim_tokens = [tokenize(c.claim_text) for c in claims]
    anom_by_id = {a.anomaly_id: a for a in anomalies}
    peers_by_anom: dict[str, list[Hypothesis]] = {}
    hypothesis_text_tokens = {h.hypothesis_id: tokenize(h.hypothesis) for h in hypotheses}
    hypothesis_tokens = {h.hypothesis_id: _hypothesis_tokens(h) for h in hypotheses}
    for h in hypotheses:
    ...
    for h in hypotheses:
        ...
        grounding = grounding_score(h, anomaly, claims_by_id)
        testability = testability_score(h)
        novelty = _novelty_score_cached(hypothesis_text_tokens[h.hypothesis_id], claim_tokens)
        cost = cost_penalty(h)
        discrim = _discriminability_score_cached(h, peers, hypothesis_tokens)
        out[h.hypothesis_id] = _build_score_breakdown(
            h,
            anomaly,
            explain,
            grounding,
