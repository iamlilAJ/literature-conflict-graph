...
_SYSTEM = (
    ...
    "research TOPIC and a list of candidate papers (id, title, abstract).\n\n"
    "First mentally decompose the TOPIC into (i) its PROBLEM / APPLICATION "
    "DOMAIN and (ii) its CORE METHOD or SUBJECT, plus any secondary qualifiers. "
    "Then rate each paper 0-3 for how directly it is ABOUT the topic:\n"
    "  3 = squarely on topic: matches the domain AND the core method/subject "
    "(and the secondary qualifiers).\n"
    "  2 = clearly related: it is centrally about the topic's CORE method or "
    "subject in a compatible domain, OR a close variant of the topic's problem "
    "-- even if it does NOT satisfy every secondary qualifier. A paper whose "
    "MAIN contribution IS the topic's core method is at least a 2, never a 1.\n"
    "  1 = tangential: only a generic technique or a single weak keyword "
    "overlaps, OR the topic's method is applied to a DIFFERENT problem/domain.\n"
    "  0 = off-topic / unrelated.\n\n"
    "Three rules that OVERRIDE surface token matches:\n"
    "  (A) DOMAIN is a near-hard gate. A paper that reuses the topic's method or "
    "keywords but solves a DIFFERENT real-world problem or domain (e.g. topic is "
    "about language models but the paper is robotics / wireless / embedded "
    "control / medicine / energy / materials / vision-only) is 0 or 1, even if "
    "its title contains several of the topic's words.\n"
    "  (B) The topic's OWN core method or subject is NOT a 'generic technique' -- "
    "do not down-score a paper for being centrally about the very method the "
    "topic names.\n"
    "  (C) Beware polysemy: judge the INTENDED sense of each term from the topic "
    "context (e.g. 'memory' as learned/experience memory vs. hardware/KV-cache "
    "memory; 'distillation' as model knowledge-distillation vs. chemical "
    "distillation). A different sense of the word is off-topic.\n"
    "Judge by the actual research problem, not shared vocabulary.\n"
