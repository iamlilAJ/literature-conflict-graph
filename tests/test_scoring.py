...
def test_low_topic_coherence_reduces_grounding_and_utility():
    claims = [
        _claim("c001", "positive", "VBMO improves multi-objective path planning."),
        _claim("c002", "negative", "Traditional path planning algorithms fail on contact-rich tasks."),
    ]
    anomaly = Anomaly(
        anomaly_id="a001",
        type="bridge_opportunity",
        central_question="Could the effect on path planning with traditional path planning algorithms transfer to multi-objective path planning with VBMO?",
        claim_ids=["c001", "c002"],
        positive_claims=["c001"],
        negative_claims=["c002"],
        shared_entities={
            "method_from": "traditional path planning algorithms",
            "task_from": "path planning",
            "method_to": "VBMO",
            "task_to": "multi-objective path planning",
        },
    )
    claims_by_id = {c.claim_id: c for c in claims}
    coherent = Hypothesis(
        hypothesis_id="h001",
        anomaly_id="a001",
        hypothesis="Baseline choice may hide whether traditional path planning algorithms transfer to VBMO on path planning tasks.",
        mechanism="Transfer depends on task overlap between traditional path planning algorithms and VBMO.",
        explains_claims=["c001", "c002"],
        predictions=["Compare matched path planning settings.", "Hold objectives fixed across both methods."],
        minimal_test="Replay both methods on the same path planning benchmark.",
    )
    off_topic = Hypothesis(
        hypothesis_id="h002",
        anomaly_id="a001",
        hypothesis="Partial test-set contamination in pretraining inflates closed-book baselines and shrinks the benefit of retrieval on standard benchmarks.",
        mechanism="Web-crawled pretraining data overlaps with benchmark questions, letting models answer without retrieval.",
        explains_claims=["c001", "c002"],
        predictions=["Use a date-filtered benchmark.", "Estimate contamination with membership inference."],
        minimal_test="Build a post-cutoff evaluation set and measure closed-book performance.",
    )
    coherent_score = _score_hypothesis(coherent, anomaly, claims_by_id, [coherent, off_topic])
    off_topic_score = _score_hypothesis(off_topic, anomaly, claims_by_id, [coherent, off_topic])
    assert coherent_score.grounding > off_topic_score.grounding
    assert coherent_score.utility > off_topic_score.utility
